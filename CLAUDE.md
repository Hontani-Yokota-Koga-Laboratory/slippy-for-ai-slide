...
2.  **サージカル（外科的）インデックス**: `_index.json` を使い、`slides.json` の必要なスライド行のみを読み書きしてください。詳細な手順は `AGENT.md` を参照。**`_index.json` は `slides.json` 保存時に自動生成されるため、手動で編集しないこと。**
...
- **`STYLE.md`**: スライド作成前の確認事項・画像プレースホルダーの使い方
