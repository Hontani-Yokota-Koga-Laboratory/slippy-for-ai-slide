...
- `projects/`: スライドデータとプロジェクト固有の資産
  ...
  - `script.yaml`: 発表者ノート（原稿）。スライド ID をキー、原稿テキストを値とするフラットマップ。
...
各プロジェクトの `script.yaml` に、スライド ID をキーとした原稿テキストを記述します。
...
- キー: `slides.json` の各スライドの `id` と一致させる
- 値: YAML リテラルブロック（`|`）で改行をそのまま記述
- API: `GET /api/projects/:name/script` → `{ [slideId]: string }` / `PUT /api/projects/:name/script/:slideId` → テキスト本文
- 発表モード: `?present=1` で起動（左: スライド全画面、右: 原稿パネル＋編集・自動保存）
